...
class EffectManager:
    ...
    def attach(self, ax) -> None:
        ...
        self.smoke_circles = self._make_circle_pool(8)
        ...
        self.shot_marks = self._make_marker_pool(ax, 8, "o", self.config.shot_color, 5)
        self.jump_marks = self._make_marker_pool(ax, 8, "^", self.config.jump_color, 11)
    ...
    def _update_projectiles(self, tick: float) -> None:
        ...
        for j in range(n):
            ...
            line.set_alpha(self.config.nade_path_alpha)
            dot.set_data([cx], [cy])
            dot.set_color(color)
    ...
    def _update_smoke(self, tick: float) -> None:
        ...
        shown = min(len(active), len(self.smoke_circles))
        ...
        for j in range(shown):
            ...
            c = self.smoke_circles[j]
            if age < grow:
                r = max_r * max(age / grow, 0.01)
                a = 0.40 * age / grow
            else:
                r = max_r
                remain = dur - age
                if remain < fade:
                    a = 0.40 * max(remain / fade, 0.0)
                else:
                    a = 0.40
            ...
            c.set_visible(True)
        for c in self.smoke_circles[shown:]:
            c.set_visible(False)
    ...
    def _update_flash(self, tick: float) -> None:
        ...
        for j in range(shown):
            ...
            c.set_alpha(max(0.9 * (1.0 - prog), 0.0))
            c.set_visible(True)
    ...
    def _update_fire(self, tick: float) -> None:
        data = self._fire
        active = self._active_idx(data, tick)
        shown = min(len(active), len(self.fire_circles))
        ...
        for j in range(shown):
            ...
            r = self.config.fire_radius * (0.8 + 0.2 * min(age / 10.0, 1.0))
            a = 0.55 if age < dur - 5 else 0.55 * max((dur - age) / 5.0, 0.0)
            c.set_center((data["px"][i], data["py"][i]))
            c.set_radius(r)
        ...
        for j in range(mshown):
            ...
            c.set_facecolor("none")
            c.set_linewidth(2.5)
            c.set_alpha(max(0.8 * (1.0 - prog), 0.0))
            c.set_visible(True)
    ...
    def _update_jump_rings(self, tick: float) -> None:
        ...
        for j in range(shown):
            ...
            c = self.jump_rings[j]
            c.set_center((data["px"][i], data["py"][i]))
            c.set_radius(18.0 + 60.0 * prog)
            c.set_alpha(max(0.7 * (1.0 - prog), 0.0))
            c.set_visible(True)
